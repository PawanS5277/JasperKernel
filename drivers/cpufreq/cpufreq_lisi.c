/*
 * drivers/cpufreq/cpufreq_lisi.c
 *
 ...
 * Author: Carlos "Klozz" Jesus (TeamMEX)
 *
 * Some part of this governor was teken from Interactive and Alessa my lovely governor
 * This governor have the nickname of a friend's daughter
 * I hope this work make it better your power management without lose power ;)
 *
 * Use trace points from interactive
 */
...
/*This is a duplicated file from interactive but is to no edit interactive one! */
#define CREATE_TRACE_POINTS
#include <trace/events/cpufreq_lisi.h> 

struct cpufreq_lisi_policyinfo {
	...
	struct cpufreq_policy nolimits; /* policy copy with no limits */
	...
	struct cpufreq_lisi_tunables *cached_tunables;
...
static DEFINE_PER_CPU(struct cpufreq_lisi_policyinfo *, polinfo);
static DEFINE_PER_CPU(struct cpufreq_lisi_cpuinfo, cpuinfo);

/* realtime thread handles frequency scaling */
static struct task_struct *speedchange_task;
static cpumask_t speedchange_cpumask;
static spinlock_t speedchange_cpumask_lock;
static struct mutex lisi_lock_gov;

static int set_window_count;
static int migration_register_count;
static struct mutex lisi_sched_lock;
static cpumask_t controlled_cpus;

/* Target load.  Lower values result in higher CPU speeds. So use 90%*/
...
struct cpufreq_lisi_tunables {
	int usage_count;
	/* Hi speed to bump to from lo speed when load burst (default max) */
	unsigned int hispeed_freq;
	/* Go to hi speed when CPU load at or above this value. */
#define DEFAULT_GO_HISPEED_LOAD 96 /* original was 99 */
...
static struct cpufreq_lisi_tunables *common_tunables;
static struct cpufreq_lisi_tunables *cached_common_tunables;

static struct attribute_group *get_sysfs_attr(void);

/* Round to starting jiffy of next evaluation window */
static u64 round_to_nw_start(u64 jif,
			     struct cpufreq_lisi_tunables *tunables)
{
...
static void cpufreq_lisi_timer_resched(unsigned long cpu,
					      bool slack_only)
{
	struct cpufreq_lisi_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_lisi_cpuinfo *pcpu;
	struct cpufreq_lisi_tunables *tunables =
 ...
 * this function.
 */
static void cpufreq_lisi_timer_start(
	struct cpufreq_lisi_tunables *tunables, int cpu)
{
	struct cpufreq_lisi_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_lisi_cpuinfo *pcpu;
...
static unsigned int freq_to_targetload(
	struct cpufreq_lisi_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
...
u32 get_max_freq_load(int cpu, unsigned int freq)
{
	struct cpufreq_lisi_policyinfo *ppol = per_cpu(polinfo, cpu);
 ...
 */
static unsigned int choose_freq(struct cpufreq_lisi_policyinfo *pcpu,
		unsigned int loadadjfreq)
{
	unsigned int freq = pcpu->policy->cur; //use the current freq
	...
	do {
         ...
         * CPUFREQ_RELATION_H 1  highest frequency below or at target 
         * CPUFREQ_RELATION_C 2  closest frequency to target 
		 * https://elixir.bootlin.com/linux/v4.3/source/include/linux/cpufreq.h#L188
		 */

		if (cpufreq_frequency_table_target(
			    &pcpu->nolimits, pcpu->freq_table, loadadjfreq / tl,
...
{
	struct cpufreq_lisi_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_lisi_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	struct cpufreq_lisi_tunables *tunables =
		ppol->policy->governor_data;
	u64 now;
	u64 now_idle;
...
static unsigned int sl_busy_to_laf(struct cpufreq_lisi_policyinfo *ppol,
				   unsigned long busy)
{
	int prev_load;
	struct cpufreq_lisi_tunables *tunables =
...
static void cpufreq_lisi_timer(unsigned long data)
{
	s64 now;
	...
	int cpu_load;
	int pol_load = 0;
	struct cpufreq_lisi_policyinfo *ppol = per_cpu(polinfo, data);
	struct cpufreq_lisi_tunables *tunables =
		ppol->policy->governor_data;
   ...
   /* When the minfreq is set to the maxfreq, there is nothing to really do: the
	* CPU is already running at its maxfreq. In this scenario, just do nothing
	* and re-arm the timer. 
    * This scenario occurs when the screen is turned on due to the cpu_input_boost
    * driver (it sets policy->min to policy->max for a few seconds).
    ...
    */
	...
	for_each_cpu(cpu, ppol->policy->cpus) {
		...
		trace_cpufreq_lisi_cpuload(cpu, cpu_load, new_load_pct,
						  prev_l, pred_l);
	...
	if (cpufreq_frequency_table_target(&ppol->nolimits, ppol->freq_table,
	...
	trace_cpufreq_lisi_target(max_cpu, pol_load, ppol->target_freq,
					 ppol->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&ppol->policy_timer))
		cpufreq_lisi_timer_resched(data, false);
...
static int cpufreq_lisi_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	struct cpufreq_lisi_policyinfo *ppol;
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			if (ppol->target_freq != ppol->policy->cur)
				__cpufreq_driver_target(ppol->policy,
							ppol->target_freq,
							CPUFREQ_RELATION_H);
			trace_cpufreq_lisi_setspeed(cpu,
						     ppol->target_freq,
						     ppol->policy->cur);
			up_read(&ppol->enable_sem);
		}
...
static void cpufreq_lisi_boost(struct cpufreq_lisi_tunables *tunables)
{
	int i;
	int anyboost = 0;
...
static enum hrtimer_restart cpufreq_lisi_hrtimer(struct hrtimer *timer)
{
	struct cpufreq_lisi_policyinfo *ppol = container_of(timer,
			struct cpufreq_lisi_policyinfo, notif_timer);
	int cpu;
	...
	cpu = ppol->notif_cpu;
	trace_cpufreq_lisi_load_change(cpu);
	del_timer(&ppol->policy_timer);
	del_timer(&ppol->policy_slack_timer);
	cpufreq_lisi_timer(cpu);
...
static int cpufreq_lisi_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_lisi_notifier,
};
...
static ssize_t show_above_hispeed_delay(
	struct cpufreq_lisi_tunables *tunables, char *buf)
{
	int i;
	ssize_t ret = 0;
	unsigned long flags;
...
static ssize_t show_hispeed_freq(struct cpufreq_lisi_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->hispeed_freq);
}

static ssize_t store_hispeed_freq(struct cpufreq_lisi_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_go_hispeed_load(struct cpufreq_lisi_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->go_hispeed_load);
}

static ssize_t store_go_hispeed_load(struct cpufreq_lisi_tunables
...
static ssize_t show_min_sample_time(struct cpufreq_lisi_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->min_sample_time);
}

static ssize_t store_min_sample_time(struct cpufreq_lisi_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct cpufreq_lisi_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate);
}

static ssize_t store_timer_rate(struct cpufreq_lisi_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val, val_round;
	struct cpufreq_lisi_tunables *t;
	int cpu;
...
static ssize_t show_timer_slack(struct cpufreq_lisi_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->timer_slack_val);
}

static ssize_t store_timer_slack(struct cpufreq_lisi_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boost(struct cpufreq_lisi_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}

static ssize_t store_boost(struct cpufreq_lisi_tunables *tunables,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (tunables->boost_val) {
		trace_cpufreq_lisi_boost("on");
		if (!tunables->boosted)
			cpufreq_lisi_boost(tunables);
	} else {
		tunables->boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_lisi_unboost("off");
	}

	return count;
}

static ssize_t store_boostpulse(struct cpufreq_lisi_tunables *tunables,
				const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	trace_cpufreq_lisi_boost("pulse");
	if (!tunables->boosted)
		cpufreq_lisi_boost(tunables);
	return count;
}

static ssize_t show_boostpulse_duration(struct cpufreq_lisi_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->boostpulse_duration_val);
}

static ssize_t store_boostpulse_duration(struct cpufreq_lisi_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_io_is_busy(struct cpufreq_lisi_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}

static ssize_t store_io_is_busy(struct cpufreq_lisi_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static int cpufreq_lisi_enable_sched_input(
			struct cpufreq_lisi_tunables *tunables)
{
	int rc = 0, j;
	struct cpufreq_lisi_tunables *t;

	mutex_lock(&lisi_sched_lock);
	...
	if (migration_register_count > 1)
		...
		atomic_notifier_chain_register(&load_alert_notifier_head,
						&load_notifier_block);
out:
	mutex_unlock(&lisi_sched_lock);
	return rc;
}

static int cpufreq_lisi_disable_sched_input(
			struct cpufreq_lisi_tunables *tunables)
{
	mutex_lock(&lisi_sched_lock);
			...
			const char *buf, size_t count)
{
	...
	if (val)
		ret = cpufreq_lisi_enable_sched_input(tunables);
	else
		ret = cpufreq_lisi_disable_sched_input(tunables);
...
show_store_gov_pol_sys(target_loads); 		 // Store target loads
show_store_gov_pol_sys(above_hispeed_delay); //The maximun high speed delay
show_store_gov_pol_sys(hispeed_freq); 		 // The High speed
show_store_gov_pol_sys(go_hispeed_load);
show_store_gov_pol_sys(min_sample_time);
show_store_gov_pol_sys(timer_rate);
show_store_gov_pol_sys(timer_slack);
show_store_gov_pol_sys(boost);
store_gov_pol_sys(boostpulse);               // Boost pulse
show_store_gov_pol_sys(boostpulse_duration); // set the boost pulse duration
show_store_gov_pol_sys(io_is_busy);          // Input/Output busy
...
static struct attribute *lisi_attributes_gov_sys[] = {
...
static struct attribute_group lisi_attr_group_gov_sys = {
	.attrs = lisi_attributes_gov_sys,
	.name = "lisi",
};

/* Per policy governor instance */
static struct attribute *lisi_attributes_gov_pol[] = {
...
static struct attribute_group lisi_attr_group_gov_pol = {
	.attrs = lisi_attributes_gov_pol,
	.name = "lisi",
};
...
{
	if (have_governor_per_policy())
		return &lisi_attr_group_gov_pol;
	else
		return &lisi_attr_group_gov_sys;
}

static void cpufreq_lisi_nop_timer(unsigned long data)
{
}

static struct cpufreq_lisi_tunables *alloc_tunable(
					struct cpufreq_policy *policy)
{
	struct cpufreq_lisi_tunables *tunables;
 ...
 * whatever it has to in order to obtain the memory requested by the     
 * caller. This flag should be your first choice.
 */
...
static struct cpufreq_lisi_policyinfo *get_policyinfo(
					struct cpufreq_policy *policy)
{
	struct cpufreq_lisi_policyinfo *ppol =
	...
	ppol->policy_timer.function = cpufreq_lisi_timer;
	init_timer(&ppol->policy_slack_timer);
	ppol->policy_slack_timer.function = cpufreq_lisi_nop_timer;
	hrtimer_init(&ppol->notif_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	ppol->notif_timer.function = cpufreq_lisi_hrtimer;
...
static struct cpufreq_lisi_tunables *get_tunables(
				struct cpufreq_lisi_policyinfo *ppol)
{
	if (have_governor_per_policy())
...
static int cpufreq_governor_lisi(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	struct cpufreq_lisi_policyinfo *ppol;
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (tunables->use_sched_load)
			cpufreq_lisi_enable_sched_input(tunables);
	...
	case CPUFREQ_GOV_POLICY_EXIT:
		...
		if (tunables->use_sched_load)
			cpufreq_lisi_disable_sched_input(tunables);

		break;

	case CPUFREQ_GOV_START:
		mutex_lock(&lisi_lock_gov);
		...
		ppol->policy = policy;
		ppol->target_freq = policy->cur;//Use the curren freq
		ppol->freq_table = freq_table; //Freqency table
		ppol->nolimits = *policy; // Policy copy no limits
		ppol->nolimits.min = policy->cpuinfo.min_freq;
		ppol->nolimits.max = policy->cpuinfo.max_freq;
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (ppol->governor_enabled) {
			if (policy->min < ppol->min_freq)
				cpufreq_lisi_timer_resched(policy->cpu,
								  true);
			ppol->min_freq = policy->min;
		}
		up_read(&ppol->enable_sem);
...
/*if we declare as default governor we need to set that function as static */
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_LISI
static
#endif
struct cpufreq_governor cpufreq_gov_lisi = {
	.name = "lisi",
	.governor = cpufreq_governor_lisi,
	.max_transition_latency = 10000000, //Make this latency sensitive
	.owner = THIS_MODULE,
};
...
static int __init cpufreq_lisi_init(void)
{
	...
	mutex_init(&lisi_sched_lock);
	speedchange_task =
		kthread_create(cpufreq_lisi_speedchange_task, NULL,
			       "cflisi");
	if (IS_ERR(speedchange_task))
		return PTR_ERR(speedchange_task);

	/* We can use SCHED_FIFO or SCHED_RR here always the task in high priority wins
     * so We need to test wich is better :D for now I will test with RR
	 */
	sched_setscheduler_nocheck(speedchange_task, SCHED_RR, &param);
...
fs_initcall(cpufreq_lisi_init);
#else
module_init(cpufreq_lisi_init);
#endif

static void __exit cpufreq_lisi_exit(void)
{
	int cpu;

	cpufreq_unregister_governor(&cpufreq_gov_lisi);
...
module_exit(cpufreq_lisi_exit);

MODULE_AUTHOR("Carlos 'Klozz' Jesús <klozz707@gmail.com>");
MODULE_DESCRIPTION("'cpufreq_lisi' - A cpufreq governor for "
	"Latency sensitive workloads and dynamic");
MODULE_LICENSE("GPL");